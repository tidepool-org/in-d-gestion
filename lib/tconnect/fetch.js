...
module.exports = function (opts, cb) {
    ...
    if (opts.interval == null) {
        ...
        opts.interval = util.format('%s/%s',
            m.clone().subtract(opts.daysAgo || 14, 'days').format('YYYY-MM-DD'),
            m.format('YYYY-MM-DD'));
    }
    ...
    [
      ...
      function (response, next) {
        ...
        if (authMatch) {
            ...
            req.body = util.format(SOAP_BODY, AuthToken, util.format(GET_THERAPY_DATA_BY_USER_ID_REQUEST, UserGUID, startTime, endTime, opts.format));
            req.setHeader('Content-Type', 'text/xml');
            req.setHeader('Accept-Encoding', 'gzip');
            req.setHeader('Content-Length', Buffer.byteLength(req.body, 'utf8'));
