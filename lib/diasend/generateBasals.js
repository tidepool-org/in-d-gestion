...
/**
 * Generates basal event records given a "settings" object and subsequent "basal-rate-change" events.
 *
 * The "settings" object must come as the first event and all subsequent events must be in order sorted
 * by `'deviceTime'`.
 *
 * This function will generate "scheduled" basal-rate-change events according to the given schedule.
 *
 * It will generate temp basals if it sees a basal-rate-change with a value that does not match the
