...
module.exports = function (opts, cb) {
  ...
  if (opts.interval == null) {
    ...
    opts.interval = util.format(
      '%s/%s', m.clone().subtract('days', opts.daysAgo || 14).format('YYYY-MM-DD'), m.format('YYYY-MM-DD')
    );
  }
    ...
    [
      ...
      function(done) {
        ...
        var o = {
          ...
          qs: {
            period: 'arbitrary',
            starttime: moment(intervalSplits[0]).format('YYYY-MM-DD'),
            endtime: moment(intervalSplits[1]).format('YYYY-MM-DD')
          }
        };
