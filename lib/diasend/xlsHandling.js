...
var configSheetHandlers = {
  'Name and glucose': function(sheet) {
    ...
    var range = xlsjs.utils.decode_range(sheet['!ref']);
    return {
      startDate: moment(getCell(sheet, 5, 0), 'DD/MM/YYYY HH:mm'),
      endDate: moment(getCell(sheet, range.e.r, 0), 'DD/MM/YYYY HH:mm')
    };
  },
  CGM: function(sheet) {
    if (getCell(sheet, 1, 0) !== 'Time') {
    ...
    var range = xlsjs.utils.decode_range(sheet['!ref']);
    return {
      startDate: moment(getCell(sheet, 2, 0), 'DD/MM/YYYY HH:mm'),
      endDate: moment(getCell(sheet, range.e.r, 0), 'DD/MM/YYYY HH:mm')
    };
  },
  'Insulin use and carbs': function(sheet) {
    ...
    var range = xlsjs.utils.decode_range(sheet['!ref']);
    return {
      startDate: moment(getCell(sheet, 1, 0), 'DD/MM/YYYY HH:mm'),
      endDate: moment(getCell(sheet, range.e.r, 0), 'DD/MM/YYYY HH:mm')
    };
  },
