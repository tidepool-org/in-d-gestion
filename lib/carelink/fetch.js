    ...
    [
      ...
      function (response, next) {
        if (response.statusCode === 403) {
          cb({error: 'badlogin',message: 'Incorrect username or password', code: 403});
          return;
        }
      ...
      function (response, next) {
        ...
        request.post(CARELINK_URLS.csv, reqOptions, next);
      },
      function(response, next) {
        ...
        if(!match) {
          cb({error: 'badrequest', message: 'We were unable to fetch data', code: 500});
        }
        ...
        if(match && match.length) {
          ...
          if(records === 0) {
            cb({error: 'norecords', message: 'No records where found for the last ' + daysAgo + 'days', code: 204});
            return;
          }
        }
