...
function makeListJoiner(setupPredicateFn, itemType) {
  return function (event) {
    ...
    return {
      ...
      completed: function () {
        throw except.ISE(
          'Incomplete %s event, ts[%s], expectedSize[%s], actual[%s]',
          itemType, setup.deviceTime, setup.size, payload.length
        );
      }
    };
  };
      ...
      handle: function(e) {
        ...
        switch (e.subType) {
          case 'activeSchedule':
            ...
            currSettings.activeSchedule = e.scheduleName;
            break;
          case 'bolusWizard':
  ...
  return function(observable) {
    return observable.link(function(observer) {
      ...
      var settingsHandlers = {
        activeSchedule: {
          onLifecycleEnd: function(e) {
            if (currSettings.activeSchedule == null) {
              currSettings.activeSchedule = e.scheduleName;
            } else if (currSettings.activeSchedule !== e.scheduleName) {
              return err(
                'basalSchedules don\'t match, [%s] !== [%s], ts[%s]',
                currSettings.activeSchedule, e.scheduleName, e.deviceTime
              );
            }
          },
          onLifecycleStart: function(e) {
            if (currSettings.activeSchedule !== e.scheduleName) {
              currSettings.activeSchedule = null;
              this.onLifecycleEnd(e);
              misc.annotateEvent(currSettings, 'carelink/settings/activeSchedule-mismatch');
            }
            ...
            currSettings.activeSchedule = e.previousSchedule;
            return retVal;
          }
        },
        bolusWizard: {
          ...
          onLifecycleStart: function(e) {
            if (! this.isUpToDate(e)) {
              this.onLifecycleEnd(e);
              misc.annotateEvent(currSettings, 'carelink/settings/wizard-mismatch');
            }
            currSettings.deviceTime = e.deviceTime;
        ...
        basalSchedule: {
          ...
          onLifecycleStart: function(e) {
            if (! _.isEqual(currSettings.basalSchedules[e.scheduleName], e.payload)) {
              this.onLifecycleEnd(e);
              misc.annotateEvent(currSettings, 'carelink/settings/basal-mismatch');
            }
      ...
      var retVal = rx.Observer.create(
        ...
        function() {
          var config = configFn();
          // Emit the schedule as it exists because Carelink actually does provide a generally clean record
          // of what the settings were as changes are made to them.  So we trust that our current settings
          // applies up until the beginning of the stream.
          observer.onNext(
            _.assign(
              {},
              _.omit(_.cloneDeep(currSettings), 'lifecycle'),
              { deviceTime: config.startTime }
            )
          );
          observer.completed();
        }
