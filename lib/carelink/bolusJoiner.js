...
function wrapHandler(handlerFn) {
  return function (event) {
    ...
    if (retVal != null) {
      return handlerWithLimitedEventBuffer(handlerOfSameUpload(retVal));
    }
    return null;
  };
}
...
function handleWizardFirst(event) {
  ...
  return {
    ...
    completed: function () {
      throw except.ISE('This should not run because handle always handles the event immediately');
    }
  };
}
...
/*
 * If we get a dual/square bolus first, then it is a dual-wave bolus with a 0 value dual/normal
 * so we change it to a plain old subType `square`.
 * The expected order is dual/square -> wizard.  We give up if we get another bolus event before a wizard.
 * NB: This code is *not* compatible with Paradigm *22 series pumps because dual-wave boluses
 * for those pumps appear in square, normal, bolus order.
 */
function handleDualSquareFirst(event) {
  ...
  function finalize(e) {
    ...
    var retVal = [
      _.assign({}, finalSquare, { subType: 'square' })
    ];
...
function handleDualNormalFirst(event) {
  ...
  function finalize(e) {
    var finalNormal = finalizeWithJoinKey(_.assign({}, normal, { subType: 'normal' }));
    ...
    if (square != null) {
      _.assign(
        finalNormal,
        { subType: 'dual/square' },
        _.pick(square, 'extended', 'expectedExtended', 'duration', 'expectedDuration')
      );
    }
    if (e != null) {
...
module.exports = function (obs) {
  return obs
    ...
    .map(function(e) {
           if (e.normal === e.expectedNormal) {
             e = _.omit(e, 'expectedNormal');
           }
           if (e.extended === e.expectedExtended) {
             e = _.omit(e, 'expectedExtended');
           }
           if (e.duration === e.expectedDuration) {
             e = _.omit(e, 'expectedDuration');
           }
           if (e.expectedExtended != null && e.expectedDuration == null) {
             e.expectedDuration = e.duration;
             e.duration = Math.floor(e.duration * (e.extended * 1.0 / e.expectedExtended));
           }
           return e;
         });
};