  ...
  return function (observable) {
    return observable
      ...
      function (outputObservable) {
        ...
        function emitSchedulesBetweenEventsIfExists(endTime) {
          if (currEvent != null) {
            ...
            if (currEvent.duration != null) {
              var durationEnd = currEvent.deviceTime.clone().add('ms', currEvent.duration);
        ...
        var equalityFields = [ 'scheduleName', 'startTime', 'value' ];
        function doScheduled(e) {
          ...
          if (_.isEqual(_.pick(currScheduled, equalityFields), _.pick(e, equalityFields)) &&
              currScheduled.deviceTime.clone().add('ms', currScheduled.duration) > e.deviceTime) {
            return;
          }

          for (var j = 0; j < schedule.length; ++j) {
            if (schedule[j].rate === e.value && e.startTime === schedule[j].start) {
              currScheduled = _.assign({}, e, { duration: determineDuration(e.deviceTime, schedule, j)});

              return emit(_.omit(currScheduled, 'startTime'));
            }
          }
          currScheduled = null;
          return emit(misc.annotateEvent(_.omit(e, 'startTime'), 'carelink/basal/off-schedule-rate'));
        }
        ...
        function fillInSchedulesForStaticBasal(newTime) {
          ...
          if (schedule != null && schedule.length === 1) {
            while (newTime.valueOf() > previousTime.valueOf()
              && newTime.dayOfYear() !== previousTime.dayOfYear()) {
              var ts = previousTime.clone().add('hours', 24);
              ts.subtract('ms', misc.computeMillisInCurrentDay(ts));
