            ...
            if (e.duration === 0) {
              if (currBasal.deliveryType === 'temp' && currBasal.suppressed != null) {
                ...
                currBasal = withPrevious(
                  _.assign({}, nextBasal, { deviceTime: e.deviceTime, time: e.time, timezoneOffset: e.timezoneOffset })
                );
          ...
          case 'scheduled':
            if (currBasal.deliveryType === 'temp') {
              ...
              if (eventTs < currTs + currBasal.duration) { // The scheduled basal is overshadowed by the temp
                ...
                currBasal = withPrevious(
                  _.assign(
                    {}, currBasal, _.pick(e, 'deviceTime', 'time', 'timezoneOffset'), {suppressed:_.omit(e, 'previous')}
                  )
                );
