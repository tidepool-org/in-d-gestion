...
exports.asNumber = function(fieldFn, defaultVal) {
  return exports.map(fieldFn, function(e) {
    ...
    if (retVal == null || Number.isNaN(retVal)) {
      throw except.ISE('Expected field[%s] to be a number, it was[%s].', fieldFn, e);
    }
    return  retVal;
  });
};
  ...
  return {
    ...
    when: function(predicate) {
      ...
      return {
         ...
         * * array - recursively apply the conversion to each element of the array and compose the resulting functions
    ...
    build: function() {
      for (var i = 0; i < rules.length; ++i) {
        if (rules[i].parserFn == null) {
          throw except.ISE('parserFn not defined on rule[%s], did you forget to call build()?', i);
        }
      }
