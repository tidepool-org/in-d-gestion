...
function testParser(dir) {
  it('should parse as expected', function (done) {
    indigestion.tconnect.parse(fs.createReadStream(dir + '/input.xml'), {timezone: 'Pacific/Honolulu'})
      .toArray()
      .subscribe(
      function(e) {
        ...
        var actualsNoId = e.map(function (element) { return _.omit(element, ['id', 'previous']); });
        ...
        expect(actualsNoId.length === e.length);
...
describe('tconnect/parse', function () {
  testParser(__dirname + '/../resources/tconnect/parse');
});
